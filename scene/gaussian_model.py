...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, previous_gaussian=None, floater_dist2_threshold=0.0002):
        ...
        if previous_gaussian is not None:
            ...
            self._scene_flow_prev = torch.cat((previous_gaussian._scene_flow.detach(), previous_gaussian._scene_flow_prev), dim=0)
            # print("previous_gaussian._scene_flow.shape", previous_gaussian._scene_flow.shape)
            # print("previous_gaussian._motion_mask.shape", previous_gaussian._motion_mask.shape)
            # print("previous_gaussian._motion_mask_prev.shape", previous_gaussian._motion_mask_prev.shape)
            # print("previous_gaussian._scene_flow_prev.shape", previous_gaussian._scene_flow_prev.shape)
            self._motion_mask_prev = torch.cat((previous_gaussian._motion_mask.detach(), previous_gaussian._motion_mask_prev), dim=0)
            print("self._scene_flow_prev.shape", self._scene_flow_prev.shape)
            print("self._motion_mask_prev.shape", self._motion_mask_prev.shape)
            ...
            self._scene_flow_prev = torch.empty(0).cuda()
            self._motion_mask_prev = torch.empty(0).cuda()
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, is_sky: bool = False):
        ...
        print("===== DEBUG create_from_pcd =====")
        print("pcd.points shape:", pts_np.shape)
        print("pcd.points dtype:", pts_np.dtype)
        print("pcd.points first rows:", pts_np[:5])

        if pts_np.size > 0:
            print("pcd min:", pts_np.min(axis=0))
            print("pcd max:", pts_np.max(axis=0))
            print("nan count:", np.isnan(pts_np).sum())
            print("inf count:", np.isinf(pts_np).sum())
        else:
        ...
        self._scene_flow = torch.tensor(np.asarray(pcd.scene_flow)).float().cuda()[~floater_mask]
        self._motion_mask = torch.tensor(np.asarray(pcd.motion_mask)).float().cuda()[~floater_mask]
    ...
    def construct_list_of_attributes(self, exclude_filter=False, use_higher_freq=True):
        l = ['x', 'y', 'z', 'dx', 'dy', 'dz', 'mx', 'my', 'mz', 'nx', 'ny', 'nz']
    ...
    def yield_splat_data(self, path):
        ...
        xyz = torch.cat([self._xyz.detach(), self._xyz_prev], dim=0).cpu().numpy()
        xyz = xyz[filter_all]
        scene_flow = torch.cat([self._scene_flow.detach(), self._scene_flow_prev], dim=0).cpu().numpy()
        scene_flow = scene_flow[filter_all]
        motion_mask = torch.cat([self._motion_mask.detach(), self._motion_mask_prev], dim=0).cpu().numpy()
        motion_mask = motion_mask[filter_all]
        ...
        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz,scene_flow, motion_mask, normals, f_dc, opacities, scale, rotation, filters_3D), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
    ...
    def save_ply_with_filter(self, path):
        ...
        scene_flow = torch.cat([self._scene_flow, self._scene_flow_prev], dim=0).detach().cpu().numpy()
        motion_mask = torch.cat([self._motion_mask, self._motion_mask_prev], dim=0).detach().cpu().numpy()
    ...
    def load_ply_with_filter(self, path):
        ...
        xyz = np.stack((np.asarray(plydata.elements[0]["x"]),
                        np.asarray(plydata.elements[0]["y"]),
                        np.asarray(plydata.elements[0]["z"])),  axis=1)
        scene_flow = np.stack((np.asarray(plydata.elements[0]["dx"]),
                        np.asarray(plydata.elements[0]["dy"]),
                        np.asarray(plydata.elements[0]["dz"])),  axis=1)
        motion_mask = np.stack((np.asarray(plydata.elements[0]["mx"]),
                        np.asarray(plydata.elements[0]["my"]),
                        np.asarray(plydata.elements[0]["mz"])),  axis=1)
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis]
        ...
        self._scene_flow = torch.tensor(scene_flow, dtype=torch.float, device="cuda").requires_grad_(True)
        self._motion_mask = torch.tensor(motion_mask, dtype=torch.float, device="cuda").requires_grad_(True)
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                if group["name"] == "scene_flow" or "motion_mask":
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_opacities, new_scaling, new_rotation, new_scene_flow, new_motion_mask):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_opacity, new_scaling, new_rotation, new_scene_flow, new_motion_mask)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_opacities, new_scaling, new_rotation, new_scene_flow, new_motion_mask)
