...
def generate_seed_preset_20(degsum=40):
    ...
    philist = np.concatenate((
        np.linspace(0, 0, 7),
        np.linspace(-12.5, -12.5, 7),
        np.linspace(12.5, 12.5, 7)
    ))
    assert len(thlist) == len(philist)  # 21

    pose_list = list(range(20))  # 20개
    render_poses = np.zeros((len(pose_list), 3, 4), dtype=np.float32)
    ...
    for idx, pose in enumerate(pose_list):
        ...
        render_poses[idx, :3, :3] = Ry @ Rx
        render_poses[idx, :3, 3:4] = 0.0  # 회전만
...
def generate_seed_preset_2updown():
    degsum = 60 
    thlist = np.concatenate((np.linspace(0, degsum, 4), np.linspace(0, -degsum, 4)[1:], np.linspace(0, degsum, 4), np.linspace(0, -degsum, 4)[1:], np.linspace(0, degsum, 4), np.linspace(0, -degsum, 4)[1:]))
    philist = np.concatenate((np.linspace(0,0,7), np.linspace(-22.5,-22.5,7), np.linspace(22.5,22.5,7)))
    assert len(thlist) == len(philist)
...
def generate_seed_preset_5views():
    # (yaw, pitch) in degrees
    angles = [(0, 0), (-5, 0), (0, 5) , (0, -5), (5, 0)]
    render_poses = np.zeros((5, 3, 4), dtype=np.float32)

    def R_yaw(y):
        yr = np.deg2rad(y)
        return np.array([[ np.cos(yr), 0, -np.sin(yr)],
                         [ 0,          1,  0          ],
                         [ np.sin(yr), 0,  np.cos(yr)]], dtype=np.float32)

    def R_pitch(p):
        pr = np.deg2rad(p)
        return np.array([[1,           0,            0          ],
                         [0,  np.cos(pr), -np.sin(pr)],
                         [0,  np.sin(pr),  np.cos(pr)]], dtype=np.float32)

    for k, (y, p) in enumerate(angles):
        R = R_yaw(y) @ R_pitch(p)          # 기존 곱셈 순서 유지
        T = np.zeros((3, 1), dtype=np.float32)  # 병진 없음
        render_poses[k, :3, :3] = R
        render_poses[k, :3, 3:4] = T
    return render_poses

def generate_seed_hemisphere_3():
    angles = [(0,-5),(0,0),(0,5)]
    d = 4.3
    render_poses = np.zeros((3,3,4))
    for i,(th,phi) in enumerate(angles):
        thr, phir = np.deg2rad(th), np.deg2rad(phi)
        R = np.matmul(
            np.array([[np.cos(thr),0,-np.sin(thr)],[0,1,0],[np.sin(thr),0,np.cos(thr)]]),
            np.array([[1,0,0],[0,np.cos(phir),-np.sin(phir)],[0,np.sin(phir),np.cos(phir)]])
        )
        T = np.array([d*np.sin(thr),0,d-d*np.cos(thr)]).reshape(3,1) \
          + np.array([0,d*np.sin(phir),d-d*np.cos(phir)]).reshape(3,1)
        render_poses[i,:3,:3] = R
        render_poses[i,:3,3:4] = T
    return render_poses

def generate_seed_hemisphere_2():
    angles = [(0,-5),(0,5)]
    d = 4.3
    render_poses = np.zeros((2,3,4))
    for i,(th,phi) in enumerate(angles):
...
def generate_seed_hemisphere_1():
    angles = [(0,0)]
    d = 4.3
    render_poses = np.zeros((1,3,4))
    for i,(th,phi) in enumerate(angles):
...
def generate_seed_hemisphere_i():
    render_poses = np.zeros((1, 3, 4), dtype=np.float32)
    render_poses[0, :3, :3] = np.eye(3, dtype=np.float32)   # R = I
    # T = 0 (이미 0으로 초기화됨)
    return render_poses
...
def get_pcdGenPoses(pcdgenpath, argdict={}):
    ...
    elif pcdgenpath == 'lookaround20':
        render_poses = generate_seed_preset_20()
    ...
    elif pcdgenpath == 'lookaround5':
        render_poses = generate_seed_preset_5views()
    elif pcdgenpath == 'lookaround3':
        render_poses = generate_seed_preset_3()
    elif pcdgenpath == 'hemisphere3':
        render_poses = generate_seed_hemisphere_3()
    elif pcdgenpath == 'lookaround2':
        render_poses = generate_seed_preset_2()
    elif pcdgenpath == 'hemisphere2':
        render_poses = generate_seed_hemisphere_2()
    elif pcdgenpath == 'lookaroundud':
        render_poses = generate_seed_preset_2updown()
    elif pcdgenpath == 'hemisphere1':
        render_poses = generate_seed_hemisphere_1()
    elif pcdgenpath == 'hemispherei':
        render_poses = generate_seed_hemisphere_i()
    elif pcdgenpath == 'lookaround1':
        render_poses = generate_seed_preset_1()
