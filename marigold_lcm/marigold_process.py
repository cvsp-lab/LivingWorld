...
class MarigoldImageProcessor(ConfigMixin):
    ...
    def resize_antialias(image: torch.Tensor, size: Tuple[int, int], mode: str, is_aa: bool = None) -> torch.Tensor:
        ...
        # bf16/half에서 bilinear/bicubic 미지원 → 일시 float32
        need_upcast = (image.dtype in (torch.bfloat16, torch.float16)) and (mode in ("bilinear", "bicubic"))
        orig_dtype = image.dtype
        if need_upcast:
            image = image.to(torch.float32)

        out = F.interpolate(image, size, mode=mode, antialias=antialias)
